...
namespace ActionPlatformer
{
	public interface IGameManager
	{
		...
		void OnInitialize(IGameManager gameManager = null);
		void OnEventFromPL(GameEvent gameEvent);
	}
	/// <summary>
	...
	public abstract class StateMachine<T> : MonoBehaviour where T : MonoBehaviour
	{
		...
		protected List<GameEvent> eventsToPL = new List<GameEvent>();
		...
		{
			if (eventsToPL.Count > 0)
			{
				for (int i = 0; i < eventsToPL.Count; i++)
				{
					SendEventsToUI(eventsToPL[i]);
				}
				eventsToPL.Clear();
			}
			CheckForStateChange();
		...
		#endregion
		protected void SendEventsToUI(GameEvent gameEvent)
		{
			if (gameController != null)
			{
				gameController.OnEventFromGameManager(gameEvent);
			}
		}
		public virtual void OnInitialize(IGameManager gameManager = null)
		{
			...
			gameController.OnInitialize(gameManager);
		}
		public void AddEventsToPL(GameEvent gameEvent)
		{
			eventsToPL.Add(gameEvent);
		}
		...
		public virtual void OnStateCompleted(State state, uint targetState = GameManager.INVALID_STATE)
		{
			if (targetState == GameManager.INVALID_STATE) return;
